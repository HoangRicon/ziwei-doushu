// app/api/charts/[id]/overview/route.ts
// POST: generate overview + save to DB (streaming)
// GET: return saved overview
...
const OVERVIEW_SYSTEM_PROMPT = `Bạn là Nhà Mệnh Lý — chuyên gia Tử Vi Đẩu Số theo hệ thống Thầy Ni Hải Hạ.
...
const OVERVIEW_PROMPT = `Hãy tạo luận giải tổng quan cho lá số này, theo cấu trúc sau:
...
// GET /api/charts/[id]/overview — return saved overview
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  return NextResponse.json({ overviewInterpretation: chart.overviewInterpretation });
}

// POST /api/charts/[id]/overview — generate + save overview (streaming)
            ...
            if (data === '[DONE]') {
              ...
              if (fullText) {
                await db.ziweiChart.update({
                  where: { id },
                  data: { overviewInterpretation: fullText },
                }).catch(err => console.error('Failed to save overview:', err));
              }
